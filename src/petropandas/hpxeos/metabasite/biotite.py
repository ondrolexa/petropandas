"""Biotite ('bi'), White, Powell, Holland, Johnson & Green (2014), from
tc-mb51NCKFMASHTO.txt (lines 1642-1749). The Mn-free core of
`hpxeos.metapelite.biotite.Biotite` (same reference, same site model minus Mn/mmbi).

M3(Mg,Fe,Fe3+,Ti,Al) M12_2(Mg,Fe) T_2(Si,Al) V_2(OH,O), M3 mult 1, M12/T mult 2. V
(hydroxyl/oxy vacancy, tied to Ti via the oxy-biotite exchange) has no cation content,
so is not part of `sites`/composition. T has a fixed 2-Si tetrahedral component
outside the mixing model (total tetrahedral cations T + fixed = 4, the standard
"AlIV = 4 - Si" convention).
...
an end-member here) is excluded from site allocation.
"""
...
# Verbatim from tc-mb51NCKFMASHTO.txt, lines 1675-1691.
_P_BLOCK = """
...
# Verbatim from tc-mb51NCKFMASHTO.txt, lines 1715-1741.
_SF_BLOCK = """
...
class Biotite(Phase):
    ...
    end_member_names: ClassVar = ["phl", "annm", "obi", "east", "tbi", "fbi"]
