"""Orthopyroxene ('opx'), White, Powell, Holland, Johnson & Green (2014) + Mn addition
White, Powell & Johnson (2014), from tc-mp51MnNCKFMASHTO.txt (lines 853-1032).

M1(Mg,Fe,Mn,Fe3+,Al) M2(Mg,Fe,Mn,Ca) T_2(Si,Al), M1/M2 mult 1, T mult 2 (the sole
tetrahedral site here - unlike chlorite/muscovite there is no extra fixed tetrahedral
component, so AlT cations = 2 - Si_total directly).

x, m, y, f, c are bulk mass-balance quantities recoverable straight from composition:
  x = Fe2+_total / (Fe2+_total + Mg_total)   (Fe, Mg only ever occupy M1/M2)
  m = Mn_total / 2                           (Mn occupies both M1 and M2 with equal
                                               site fraction m, same as chlorite's Mn:
                                               Mn_total = m*1 + m*1 = 2m)
...
from site allocation, matching the project's garnet/chlorite/muscovite convention.
"""
...
# Verbatim from tc-mp51MnNCKFMASHTO.txt, lines 902-928.
_P_BLOCK = """
...
# Verbatim from tc-mp51MnNCKFMASHTO.txt, lines 964-1001.
_SF_BLOCK = """
