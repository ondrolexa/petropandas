"""K-feldspar ('k4tr'), Holland, Green & Powell (2021) "4TR" ternary feldspar model
with K-feldspar-friendly parameterisation, from tc-mp51MnNCKFMASHTO.txt (lines 191-249).

A(Na,Ca,K) TB_4(Al,Si), A mult 1, TB mult 4 (1/4 entropy-of-mixing convention on TB).

No hidden order-disorder parameter: na = xNaA and ca = xCaA are exactly recoverable
from bulk composition (A site only ever holds Na, Ca, K), so Al/Si (the TB site) carry
no information needed for na, ca or the end-member proportions - though they're still
required input columns, since a real feldspar analysis should report them.
"""
...
# Verbatim from tc-mp51MnNCKFMASHTO.txt, lines 221-223.
_P_BLOCK = """
p(ab)   1 1    0 1  1 na
p(an)   1 1    0 1  1 ca
...
# Verbatim from tc-mp51MnNCKFMASHTO.txt, lines 237-241.
_SF_BLOCK = """
xNaA       1 1    0 1  1 na
xCaA       1 1    0 1  1 ca
...
class KFeldspar(Phase):
    ...
    # -- petropandas Mineral metadata (from old TC_k4tr) --
    n_oxygens = 8
    ideal_cations = None
    ...
    def variables(
        ...
        return pd.DataFrame(
            {"na": site_fractions["xNaA"], "ca": site_fractions["xCaA"]},
