...
# ---------------------------------------------------------------------------
# THERMOCALC end-member polynomial evaluation
# ---------------------------------------------------------------------------

# A parsed factor is (flag, [(coeff, var_name), ...]); a parsed term is a
# list of factors; parsed_terms is a list of terms. Evaluating a factor
# means flag + sum(coeff * variables[var_name] for coeff, var_name in pairs).
_ParsedFactor = tuple[int, list[tuple[float, str]]]
_ParsedTerm = list[_ParsedFactor]


def _parse_em_tokens(tokens: tuple) -> list[_ParsedTerm]:
    """Parse a flat THERMOCALC token tuple into nested terms/factors.

    The token format is a flat list produced by THERMOCALC's axe helper.
    Each ``p(em)`` block starts with ``NumTerms`` followed by term
    definitions. Each term starts with ``NumFactors`` followed by factor
    definitions:

    * **Flag 0, Count 1** – single-variable factor: ``0  1  <coeff>  <var>``
    * **Flag 1, Count N** – bracket factor ``(1 + Σ coeff*var)``:
      ``1  N  <coeff₁> <var₁> … <coeffₙ> <varₙ>``
    * **Flag 0, Count N** – bracket-from-zero ``(0 + Σ coeff*var)``:
      ``0  N  <coeff₁> <var₁> … <coeffₙ> <varₙ>``

    A term's value is the product of its factors; the end-member value is
    the sum of all terms. Uses index-based traversal (no ``list.pop(0)``),
    so parsing a token list of length L is O(L) rather than the O(L^2) cost
    of repeated front-pops.

    Args:
        tokens: Flat token tuple (strings or ints).

    Returns:
        List of terms; each term is a list of ``(flag, [(coeff, var), ...])``
        factors.
    """
    i = 0
    num_terms = int(tokens[i])
    i += 1
    terms: list[_ParsedTerm] = []
    for _ in range(num_terms):
        num_factors = int(tokens[i])
        i += 1
        factors: _ParsedTerm = []
        for _ in range(num_factors):
            flag, count = int(tokens[i]), int(tokens[i + 1])
            i += 2
            pairs: list[tuple[float, str]] = []
            for _ in range(count):
                coeff = float(Fraction(str(tokens[i])))
                var_name = tokens[i + 1]
                pairs.append((coeff, var_name))
                i += 2
            factors.append((flag, pairs))
        terms.append(factors)
    return terms


_parse_em_tokens_cached = lru_cache(maxsize=None)(_parse_em_tokens)


def _evaluate_em_tokens(
    parsed_terms: list[_ParsedTerm], variables: dict[str, pd.Series], index: pd.Index
) -> pd.Series:
    """Evaluate a parsed THERMOCALC polynomial across all rows at once.

    Args:
        parsed_terms: Output of :func:`_parse_em_tokens`.
        variables: Mixing variables, each a full-column ``pd.Series``.
        index: Index to align intermediate Series on.

    Returns:
        Evaluated end-member proportion per row (raw, before normalisation).
    """
    total = pd.Series(0.0, index=index)
    for factors in parsed_terms:
        term_value = pd.Series(1.0, index=index)
        for flag, pairs in factors:
            factor_value = pd.Series(float(flag), index=index)
            for coeff, var_name in pairs:
                factor_value = factor_value + coeff * variables[var_name]
...
class Mineral:
    ...
    # THERMOCALC-style end-member computation (optional)
    mixing_variables: dict | None = None
    end_member_defs: list | None = None
    ...
    def end_members(self, df: pd.DataFrame, units: str = "wt%") -> pd.DataFrame:
        ...
        When ``end_member_defs`` is set (THERMOCALC-style), computes
        end-members by evaluating each end-member's polynomial mixing-model
        tokens against the mixing variables.  Otherwise delegates to the
        subclass override.
        ...
        """
        if self.end_member_defs is not None:
            raw = self._raw_apfu(df, units)
            return self._compute_end_members(raw)
        msg = f"{type(self).__name__} does not define end-member calculations"
        raise NotImplementedError(msg)

    # -- internal helpers ----------------------------------------------------

    def _compute_mixing_variables(self, raw_apfu: pd.DataFrame) -> dict[str, pd.Series]:
        """Compute mixing variables from raw APFU.

        Each mixing variable is a callable that takes the raw APFU
        DataFrame and returns a ``pd.Series``.

        Args:
            raw_apfu: DataFrame with ion-named columns.

        Returns:
            Dict mapping variable names to Series.
        """
        if self.mixing_variables is None:
            return {}
        return {name: fn(raw_apfu) for name, fn in self.mixing_variables.items()}

    def _compute_end_members(self, raw_apfu: pd.DataFrame) -> pd.DataFrame:
        """Compute end-member percentages.

        Each ``end_member_defs`` entry is ``(name, [token_list])`` where the
        token list is a flat THERMOCALC polynomial, parsed once (cached) by
        :func:`_parse_em_tokens` and evaluated across all rows at once by
        :func:`_evaluate_em_tokens`.

        Results are normalised to sum to 100 % per row.
        ...
        Returns:
            DataFrame of end-member percentages.
        """
        mv = self._compute_mixing_variables(raw_apfu)
        idx = raw_apfu.index
        result: dict[str, pd.Series] = {}

        for name, tokens in self.end_member_defs:
            parsed = _parse_em_tokens_cached(tuple(tokens))
            result[name] = _evaluate_em_tokens(parsed, mv, idx)

        df = pd.DataFrame(result, index=idx)
        total = df.sum(axis=1).replace(0, 1)
        return (df.div(total, axis=0) * 100).round(3)
...
# ---------------------------------------------------------------------------
# THERMOCALC-compatible mineral instances (tc-mp51.txt)
#
# End-member proportions are computed by evaluating THERMOCALC polynomial
# tokens directly against mixing variables (no site-fraction intermediaries).
# Each p(em) block from tc-mp51.txt is stored as a flat token list.
# Subclasses TC_Epidote, TC_Chlorite, TC_Spinel override _raw_apfu for
# mineral-specific APFU normalisation.
# ---------------------------------------------------------------------------

# ---- Garnet (CFMMnASO) ---------------------------------------------------
# White, Powell, Holland, Johnson & Green (2014); White, Powell & Johnson (2014)
# fmt: off
TC_g = Mineral(
    name="TC Garnet",
    n_oxygens=12,
    ...
    site_definitions=[
        {"name": "Z", "capacity": 3.0, "priority": ["Si{4+}", "Al{3+}"]},
        {"name": "Y", "capacity": 2.0, "priority": ["Al{3+}", "Ti{4+}", "Cr{3+}", "Fe{3+}"]},
        {"name": "X", "capacity": 3.0, "priority": ["Fe{2+}", "Mg{2+}", "Ca{2+}", "Mn{2+}"]},
    ],
    mixing_variables={
        "x": lambda a: a["Fe{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "z": lambda a: a["Ca{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Ca{2+}"] + a["Mn{2+}"]).replace(0, 1),
        "m": lambda a: a["Mn{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Ca{2+}"] + a["Mn{2+}"]).replace(0, 1),
        "f": lambda a: a.get("Fe{3+}", pd.Series(0.0, index=a.index)),
    },
    end_member_defs=[
        ("py", [3, 1, 1, 4, -1, "f", -1, "m", -1, "x", -1, "z",
                2, 0, 1, 1, "m", 0, 1, 1, "x",
                2, 0, 1, 1, "x", 0, 1, 1, "z"]),
        ("alm", [3, 1, 0, 1, 1, "x", 2, 0, 1, -1, "m", 0, 1, 1, "x",
                 2, 0, 1, -1, "x", 0, 1, 1, "z"]),
        ("spss", [1, 1, 0, 1, 1, "m"]),
        ("gr", [1, 1, 0, 1, 1, "z"]),
        ("kho", [1, 1, 0, 1, 1, "f"]),
...
# ---- Ternary Feldspar — plagioclase-friendly (NCKAlSi3O8) ---------------
# Holland, Green & Powell (2021)

# fmt: off
TC_pl4tr = Mineral(
    name="TC Feldspar (pl4tr)",
    n_oxygens=8,
    ...
    mixing_variables={
        "na": lambda a: (
            a["Na{+}"] / (a["Na{+}"] + a["Ca{2+}"] + a["K{+}"]).replace(0, 1)
        ),
        "ca": lambda a: (
            a["Ca{2+}"] / (a["Na{+}"] + a["Ca{2+}"] + a["K{+}"]).replace(0, 1)
        ),
        "k": lambda a: a["K{+}"] / (a["Na{+}"] + a["Ca{2+}"] + a["K{+}"]).replace(0, 1),
    },
    end_member_defs=[
        ("ab", [1, 1, 0, 1, 1, "na"]),
        ("an", [1, 1, 0, 1, 1, "ca"]),
        ("san", [1, 1, 1, 2, -1, "na", -1, "ca"]),
    ],
)
# fmt: on

# ---- Ternary Feldspar — K-feldspar-friendly (NCKAlSi3O8) ----------------
# Holland, Green & Powell (2021) — same model, different parameterisation

# fmt: off
TC_k4tr = Mineral(
    name="TC Feldspar (k4tr)",
    n_oxygens=8,
    analytical_total_range=(98.5, 101.5),
    site_definitions=[
        {"name": "A", "capacity": 1.0, "priority": ["Na{+}", "Ca{2+}", "K{+}"]},
...
# ---- Cordierite (MnFMASH) ------------------------------------------------
# White, Powell, Holland, Johnson & Green (2014)
...
TC_cd = Mineral(
    ...
    mixing_variables={
        "x": lambda a: a["Fe{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "m": lambda a: (
            a.get("Mn{2+}", pd.Series(0.0, index=a.index))
            / (
                a["Fe{2+}"]
                + a["Mg{2+}"]
                + a.get("Mn{2+}", pd.Series(0.0, index=a.index))
        ...
        "h": lambda a: pd.Series(0.0, index=a.index),
    },
    end_member_defs=[
        ("crd", [2, 1, 1, 3, -1, "h", -1, "m", -1, "x", 2, 0, 1, 1, "m", 0, 1, 1, "x"]),
        ("fcrd", [2, 1, 0, 1, 1, "x", 2, 0, 1, -1, "m", 0, 1, 1, "x"]),
        ("hcrd", [1, 1, 0, 1, 1, "h"]),
        ("mncd", [1, 1, 0, 1, 1, "m"]),
    ],
)
...
TC_ctd = Mineral(
    ...
    mixing_variables={
        "x": lambda a: a["Fe{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "m": lambda a: (
            a["Mn{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Mn{2+}"]).replace(0, 1)
        ),
        "f": lambda a: a.get("Fe{3+}", pd.Series(0.0, index=a.index)),
    },
    end_member_defs=[
        (
            "mctd",
            [2, 1, 1, 3, -1, "f", -1, "m", -1, "x", 2, 0, 1, 1, "m", 0, 1, 1, "x"],
        ),
        ("fctd", [2, 1, 0, 1, 1, "x", 2, 0, 1, -1, "m", 0, 1, 1, "x"]),
        ("mnct", [1, 1, 0, 1, 1, "m"]),
        ("ctdo", [1, 1, 0, 1, 1, "f"]),
    ],
)
...
# ---- Epidote (CFASHO) — all Fe as Fe3+ -----------------------------------
# Holland & Powell (2011)


# fmt: off
class TC_Epidote(Mineral):
    """TC Epidote — all Fe treated as Fe3+ (converts FeO → Fe2O3)."""
    ...
    mixing_variables = {
        "f": lambda a: (a.get("Fe{3+}", pd.Series(0.0, index=a.index))
            + a.get("Fe{3+}", pd.Series(0.0, index=a.index))) / 2,
        "Q": lambda a: pd.Series(0.0, index=a.index),
    }
    end_member_defs = [
        ("cz", [1, 1, 1, 2, -1, "f", -1, "Q"]),
        ("ep", [1, 1, 0, 1, 2, "Q"]),
    ...
    def _preprocess_oxides(self, oxide_df: pd.DataFrame) -> pd.DataFrame:
        ...
        work = oxide_df.copy()
        cols = _oxide_cols(work)
        if "FeO" in cols and "Fe2O3" not in cols:
            work["Fe2O3"] = work.pop("FeO") * 1.11134
        elif "FeO" in cols and "Fe2O3" in cols:
            work["Fe2O3"] = work["Fe2O3"] + work.pop("FeO") * 1.11134
        elif "FeO" in cols:
            work.pop("FeO")
        return work
...
TC_ma = Mineral(
    ...
    mixing_variables={
        "x": lambda a: a["Fe{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "y": lambda a: (
            a["Al{3+}"] / (a["Mg{2+}"] + a["Fe{2+}"] + a["Al{3+}"]).replace(0, 1)
        ),
        "f": lambda a: a.get("Fe{3+}", pd.Series(0.0, index=a.index)),
        "n": lambda a: (
            a["Na{+}"] / (a["K{+}"] + a["Na{+}"] + a["Ca{2+}"]).replace(0, 1)
        ),
        "c": lambda a: (
            a["Ca{2+}"] / (a["K{+}"] + a["Na{+}"] + a["Ca{2+}"]).replace(0, 1)
        ),
    },
    end_member_defs=[
        ("mut", [1, 1, 0, 4, -1, "c", -1, "f", -1, "n", 1, "y"]),
        ("celt", [2, 1, 1, 2, -1, "x", -1, "y", 2, 0, 1, 1, "x", 0, 1, 1, "y"]),
        ("fcelt", [2, 1, 0, 1, 1, "x", 2, 0, 1, -1, "x", 0, 1, 1, "y"]),
        ("pat", [1, 1, 0, 1, 1, "n"]),
        ("ma", [1, 1, 0, 1, 1, "c"]),
        ("fmu", [1, 1, 0, 1, 1, "f"]),
    ],
...
TC_mu = Mineral(
    ...
    end_member_defs=[
        ("mu", [1, 1, 0, 4, -1, "c", -1, "f", -1, "n", 1, "y"]),
        ("cel", [2, 1, 1, 2, -1, "x", -1, "y", 2, 0, 1, 1, "x", 0, 1, 1, "y"]),
        ("fcel", [2, 1, 0, 1, 1, "x", 2, 0, 1, -1, "x", 0, 1, 1, "y"]),
        ("pa", [1, 1, 0, 1, 1, "n"]),
        ("mat", [1, 1, 0, 1, 1, "c"]),
        ("fmu", [1, 1, 0, 1, 1, "f"]),
    ],
...
TC_st = Mineral(
    ...
    mixing_variables={
        "x": lambda a: a["Fe{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "m": lambda a: a["Mn{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Mn{2+}"]).replace(0, 1),
        "f": lambda a: a.get("Fe{3+}", pd.Series(0.0, index=a.index)),
        "t": lambda a: a["Ti{4+}"] / (a["Al{3+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index)) + a["Ti{4+}"]).replace(0, 1),
    },
    end_member_defs=[
        ("mstm", [2, 1, 1, 4, -1, "f", -1, "m", -1, "x", "-4/3", "t",
                  2, 0, 1, 1, "m", 0, 1, 1, "x"]),
        ("fst", [2, 1, 0, 1, 1, "x", 2, 0, 1, -1, "m", 0, 1, 1, "x"]),
        ("mnstm", [1, 1, 0, 1, 1, "m"]),
        ("msto", [1, 1, 0, 1, 1, "f"]),
        ("mstt", [1, 1, 0, 1, "4/3", "t"]),
    ],
...
# ---- Spinel (FMATO) — merge Fe2O3 into FeO, then Droop ------------------
# White, Powell & Clarke (2002)


# fmt: off
class TC_Spinel(Mineral):
    """TC Spinel — merges Fe2O3 into FeO before APFU, then Droop split."""
    ...
    mixing_variables = {
        "x": lambda a: a["Fe{2+}"] / (a["Mg{2+}"] + a["Fe{2+}"]).replace(0, 1),
        "y": lambda a: a["Al{3+}"] / (a["Al{3+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index)) + a["Ti{4+}"]).replace(0, 1),
        "z": lambda a: 2 * a["Ti{4+}"] / (a["Al{3+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index)) + 2 * a["Ti{4+}"]).replace(0, 1),
    }
    end_member_defs = [
        ("herc", [2, 1, 0, 1, 1, "y", 2, -1, 1, 1, "x", 1, 1, 1, "z"]),
        ("sp", [1, 2, 1, 1, -1, "x", 1, 1, 1, "z"]),
        ("mt", [1, 1, 1, 2, -1, "y", -1, "z"]),
        ("usp", [1, 1, 0, 1, 1, "z"]),
    ]
    ...
    def _preprocess_oxides(self, oxide_df: pd.DataFrame) -> pd.DataFrame:
        ...
        cols = _oxide_cols(work)
        if "Fe2O3" in cols:
            feo = work.pop("Fe2O3") * 0.89978
            if "FeO" in cols:
                work["FeO"] = work["FeO"] + feo
            else:
                work["FeO"] = feo
        return work
...
TC_ilmm = Mineral(
    ...
    mixing_variables={
        "i": lambda a: 1 - a.get("Fe{3+}", pd.Series(0.0, index=a.index)),
        "g": lambda a: (
            a["Mg{2+}"]
            / (
                a["Fe{2+}"]
                ...
                + a["Mg{2+}"]
                + a["Mn{2+}"]
                + a.get("Fe{3+}", pd.Series(0.0, index=a.index))
            ).replace(0, 1)
        ),
        "m": lambda a: (
            a["Mn{2+}"]
...
TC_bi = Mineral(
    ...
    mixing_variables={
        "x": lambda a: (
            (2 * a["Fe{2+}"]) / (2 * a["Fe{2+}"] + 2 * a["Mg{2+}"]).replace(0, 1)
        ),
        "m": lambda a: (
            a["Mn{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Mn{2+}"]).replace(0, 1)
        ),
        "y": lambda a: (
            a["Al{3+}"]
            / (
                a["Mg{2+}"] + a["Fe{2+}"] + a["Al{3+}"] + a["Ti{4+}"] + a["Mn{2+}"]
            ).replace(0, 1)
        ),
        "f": lambda a: pd.Series(0.0, index=a.index),
        "t": lambda a: (
            a["Ti{4+}"]
            / (
                a["Mg{2+}"] + a["Fe{2+}"] + a["Al{3+}"] + a["Ti{4+}"] + a["Mn{2+}"]
    ...
    end_member_defs=[
        ("phl", [5, 1, 1, 6, -1, "f", -1, "m", -1, "t", -1, "x", -1, "y",
                 "-2/3", "Q", 2, 0, 1, 1, "f", 0, 1, 1, "x",
                 2, 0, 1, 3, "m", 0, 1, 1, "x",
                 2, 0, 1, 1, "t", 0, 1, 1, "x",
                 2, 0, 1, 1, "x", 0, 1, 1, "y"]),
        ("annm", [1, 1, 0, 2, "-1/3", "Q", 1, "x"]),
        ("obi", [5, 1, 0, 1, 1, "Q",
                 2, 0, 1, -1, "f", 0, 1, 1, "x",
                 2, 0, 1, -3, "m", 0, 1, 1, "x",
                 2, 0, 1, -1, "t", 0, 1, 1, "x",
...
# fmt: off
TC_opx = Mineral(
    name="TC Orthopyroxene",
    n_oxygens=6,
    ideal_cations=4,
    analytical_total_range=(99.0, 101.0),
    valence_splits=[{"element": "Fe", "method": "droop"}],
    site_definitions=[
        {"name": "M1", "capacity": 1.0, "priority": ["Mg{2+}", "Fe{2+}", "Mn{2+}", "Fe{3+}", "Al{3+}"]},
        {"name": "M2", "capacity": 1.0, "priority": ["Mg{2+}", "Fe{2+}", "Mn{2+}", "Ca{2+}"]},
        {"name": "T", "capacity": 2.0, "priority": ["Si{4+}", "Al{3+}"]},
    ],
    mixing_variables={
        "x": lambda a: a["Fe{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "m": lambda a: a["Mn{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Mn{2+}"]).replace(0, 1),
        "y": lambda a: a["Al{3+}"] / (a["Mg{2+}"] + a["Fe{2+}"] + a["Mn{2+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index)) + a["Al{3+}"]).replace(0, 1),
        "f": lambda a: a.get("Fe{3+}", pd.Series(0.0, index=a.index)) / (a["Mg{2+}"] + a["Fe{2+}"] + a["Mn{2+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index)) + a["Al{3+}"]).replace(0, 1),
        "c": lambda a: a["Ca{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Mn{2+}"] + a["Ca{2+}"]).replace(0, 1),
        "Q": lambda a: pd.Series(0.0, index=a.index),
    },
    end_member_defs=[
        ("en", [5, 1, 1, 6, "-1/2", "Q", -1, "c", -1, "f", -1, "m", -1, "x", -1, "y",
                2, 0, 1, "1/2", "c", 0, 1, 1, "Q",
                2, 0, 1, "1/2", "m", 0, 1, 1, "Q",
                2, 0, 1, 1, "c", 0, 1, 1, "x",
                2, 0, 1, 1, "m", 0, 1, 1, "x"]),
        ("fs", [6, 1, 0, 2, "-1/2", "Q", 1, "x",
                2, 0, 1, "1/2", "c", 0, 1, 1, "Q",
                2, 0, 1, "1/2", "m", 0, 1, 1, "Q",
                2, 0, 1, -1, "f", 0, 1, 1, "x",
                2, 0, 1, -1, "m", 0, 1, 1, "x",
...
TC_sa = Mineral(
    ...
    valence_splits=[{"element": "Fe", "method": "droop"}],
    site_definitions=[
        {"name": "M3", "capacity": 1.0, "priority": ["Mg{2+}", "Fe{2+}", "Fe{3+}", "Al{3+}"]},
        {"name": "M456", "capacity": 3.0, "priority": ["Mg{2+}", "Fe{2+}"]},
        {"name": "T", "capacity": 1.0, "priority": ["Si{4+}", "Al{3+}"]},
    ],
    mixing_variables={
        "x": lambda a: (a["Fe{2+}"]) / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "y": lambda a: a["Al{3+}"] / (a["Mg{2+}"] + a["Fe{2+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index)) + a["Al{3+}"]).replace(0, 1),
        "f": lambda a: a.get("Fe{3+}", pd.Series(0.0, index=a.index)) / (a["Mg{2+}"] + a["Fe{2+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index)) + a["Al{3+}"]).replace(0, 1),
        "Q": lambda a: pd.Series(0.0, index=a.index),
    },
    end_member_defs=[
        ("spr4", [1, 1, 1, 4, "-1/4", "Q", -1, "f", -1, "x", -1, "y"]),
        ("spr5", [1, 1, 0, 1, 1, "y"]),
        ("fspm", [3, 1, 0, 2, 1, "x", "-3/4", "Q",
                  2, 0, 1, -1, "f", 0, 1, 1, "x",
                  2, 0, 1, -1, "x", 0, 1, 1, "y"]),
        ("spro", [3, 1, 0, 1, 1, "Q",
                  2, 0, 1, 1, "f", 0, 1, 1, "x",
...
# Green, White, Diener, Powell, Holland & Palin (2016)

# fmt: off
TC_dio = Mineral(
    name="TC Clinopyroxene",
    n_oxygens=6,
    ideal_cations=4,
    analytical_total_range=(99.0, 101.0),
    valence_splits=[{"element": "Fe", "method": "droop"}],
    site_definitions=[
        {"name": "M1", "capacity": 1.0, "priority": ["Mg{2+}", "Fe{2+}", "Fe{3+}", "Al{3+}"]},
        {"name": "M2", "capacity": 1.0, "priority": ["Na{+}", "Ca{2+}"]},
        {"name": "T", "capacity": 2.0, "priority": ["Si{4+}", "Al{3+}"]},
    ],
    mixing_variables={
        "x": lambda a: (a["Fe{2+}"]) / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "f": lambda a: a.get("Fe{3+}", pd.Series(0.0, index=a.index)) / (a["Al{3+}"] + a.get("Fe{3+}", pd.Series(0.0, index=a.index))).replace(0, 1),
        "j": lambda a: a["Na{+}"] / (a["Na{+}"] + a["Ca{2+}"]).replace(0, 1),
        "Q": lambda a: pd.Series(0.0, index=a.index),
        "Qaf": lambda a: pd.Series(0.0, index=a.index),
        "Qfm": lambda a: pd.Series(0.0, index=a.index),
    },
    end_member_defs=[
        ("jd", [2, 1, 0, 3, 1, "j", -1, "Q", -1, "Qaf",
                2, 0, 1, -1, "f", 0, 1, 1, "j"]),
        ("di", [5, 1, 1, 4, -1, "j", -1, "Q", 1, "Qfm", -1, "x",
                2, 0, 1, -1, "j", 0, 1, 1, "Qfm",
                2, 0, 1, -1, "Q", 0, 1, 1, "Qfm",
                2, 0, 1, 1, "j", 0, 1, 1, "x",
                2, 0, 1, -1, "Q", 0, 1, 1, "x"]),
        ("hed", [5, 1, 0, 2, 1, "Qfm", 1, "x",
                 2, 0, 1, -1, "j", 0, 1, 1, "Qfm",
                 2, 0, 1, -1, "Q", 0, 1, 1, "Qfm",
                 2, 0, 1, -1, "j", 0, 1, 1, "x",
                 2, 0, 1, -1, "Q", 0, 1, 1, "x"]),
...
# fmt: off
class TC_Chlorite(Mineral):
    """TC Chlorite -- charge-based APFU normalisation (28 charges)."""
    ...
    site_definitions = [
        {"name": "T", "capacity": 4.0, "priority": ["Si{4+}", "Al{3+}"]},
        {"name": "M", "capacity": 6.0, "priority": ["Mg{2+}", "Fe{2+}", "Al{3+}", "Mn{2+}"]},
    ]
    mixing_variables = {
        "x": lambda a: (a["Fe{2+}"]) / (a["Fe{2+}"] + a["Mg{2+}"]).replace(0, 1),
        "y": lambda a: a["Al{3+}"] / (a["Mg{2+}"] + a["Fe{2+}"] + a["Al{3+}"]).replace(0, 1),
        "f": lambda a: pd.Series(0.0, index=a.index),
        "m": lambda a: a["Mn{2+}"] / (a["Fe{2+}"] + a["Mg{2+}"] + a["Mn{2+}"]).replace(0, 1),
        "QAl": lambda a: pd.Series(0.0, index=a.index),
        "Q1": lambda a: pd.Series(0.0, index=a.index),
        "Q4": lambda a: pd.Series(0.0, index=a.index),
    }
    end_member_defs = [
        ("clin", [11, 1, 0, 4, "-1/4", "Q1", -1, "m", 2, "QAl", "-5/4", "Q4",
                  ...
                  2, 0, 1, -1, "x", 0, 1, 1, "y"]),
        ("afchl", [10, 1, 1, 6, -1, "f", -1, "QAl", -1, "y", -2, "x", "5/4", "Q1", "9/4", "Q4",
                   ...
                   2, 0, 1, 3, "x", 0, 1, 1, "y"]),
        ("ames", [1, 1, 0, 2, -1, "QAl", 1, "y"]),
        ("daph", [11, 1, 0, 2, "1/4", "Q1", "5/4", "Q4",
                  2, 0, 1, "-1/4", "m", 0, 1, 1, "Q1",
                  2, 0, 1, "-5/4", "f", 0, 1, 1, "Q4",
                  2, 0, 1, "1/4", "Q1", 0, 1, 1, "QAl",
                  2, 0, 1, "-5/4", "Q4", 0, 1, 1, "QAl",
        ...
        ("ochl1", [7, 1, 0, 2, -1, "Q4", 1, "x",
                   ...
                   2, 0, 1, 1, "Q4", 0, 1, 1, "y",
                   2, 0, 1, -1, "x", 0, 1, 1, "y"]),
        ("ochl4", [9, 1, 0, 3, 1, "x", "-5/4", "Q1", "-5/4", "Q4",
                   2, 0, 1, "5/4", "m", 0, 1, 1, "Q1",
                   2, 0, 1, "5/4", "f", 0, 1, 1, "Q4",
                   2, 0, 1, "-5/4", "Q1", 0, 1, 1, "QAl",
                   2, 0, 1, "5/4", "Q4", 0, 1, 1, "QAl",
