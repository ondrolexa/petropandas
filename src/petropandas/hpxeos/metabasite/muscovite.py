...
tc-mb51NCKFMASHTO.txt (lines 1773-1885). Numerically identical to
`hpxeos.metapelite.muscovite.Muscovite` (same site model, same mass balance, same
W/a values) - the only difference anywhere in the block is that the Ca end-member is
named "mam" here instead of metapelite's "mat". See `DioctahedralMica`
(`hpxeos.dioctahedral_mica`) for the shared site model this and metapelite's
`Muscovite`/`Margarite` all use.
"""
...
# Verbatim from tc-mb51NCKFMASHTO.txt, lines 1814-1826.
_P_BLOCK = """
...
# Verbatim from tc-mb51NCKFMASHTO.txt, lines 1854-1874.
_SF_BLOCK = """
xKA        1 1    1  2  -1  c  -1  n
...
class Muscovite(DioctahedralMica):
    abbreviation = "mu"
    end_member_names: ClassVar = ["mu", "cel", "fcel", "pa", "mam", "fmu"]
