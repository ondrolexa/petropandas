"""Ilmenite-hematite ('ilmm'), White, Powell, Holland, Johnson & Green (2014), from
tc-mb51NCKFMASHTO.txt (lines 1493-1576). The Mn-free core of
`hpxeos.metapelite.ilmenite_mixed.IlmeniteMixed` (same reference, same site model
minus Mn/pnt). ("This model may give implausibly high Mg contents, in which case the
older, Mg-free model `Ilmenite` ('ilm') would be preferable" - axfile's own note.)

A(Fe,Ti,Mg,Fe3+) B(Fe,Ti,Fe3+), both mult 1. Fe and Ti are shared between A and B (not
independently resolvable from bulk composition), but Mg is confined to A alone, and
Fe3+ is tied identically between the two sites (xFe3A = xFe3B, both equal 1-i), so no
Al-style "fill one site first" convention is needed here.

i, g are bulk mass-balance quantities:
...
# Verbatim from tc-mb51NCKFMASHTO.txt, lines 1534-1540.
_P_BLOCK = """
p(oilm)    1 1    0  1  1  Q
...
# Verbatim from tc-mb51NCKFMASHTO.txt, lines 1553-1565.
_SF_BLOCK = """
...
class IlmeniteMixed(Phase):
    abbreviation = "ilmm"
    sites: ClassVar = {
        "A": ["Fe{2+}", "Ti{4+}", "Mg{2+}", "Fe{3+}"],
        "B": ["Fe{2+}", "Ti{4+}", "Fe{3+}"],
    }
    optional_columns: ClassVar = {"Fe{3+}"}
    end_member_names: ClassVar = ["oilm", "dilm", "dhem", "geik"]
